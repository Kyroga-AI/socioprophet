...
export async function fetchFxLive(): Promise<Map<string, CryptoQuote> | null> {
  try {
    ...
    const latest = await fetch(`https://api.frankfurter.dev/v1/latest?${q}`, { headers: { accept: 'application/json' } });
    ...
    try {
      ...
      const p = await fetch(`https://api.frankfurter.dev/v1/${d.toISOString().slice(0, 10)}?${q}`, { headers: { accept: 'application/json' } });
